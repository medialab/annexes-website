<script lang="ts">
	import { currentPanel, isMobile } from '$lib/stores';
...
>
	<div
		class="z-2 flex w-full origin-center flex-col-reverse justify-center gap-4 px-4 text-[#444444] md:h-full md:max-w-[60%] md:flex-row md:px-0 xl:max-w-[80%]"
	>
		{#if $currentPanel}
			<Navigator></Navigator>
		{/if}
		<div
			id="viewer"
			class="flex h-full w-full flex-1 flex-col items-start justify-between overflow-y-scroll rounded-3xl border-0 bg-white p-4 md:h-full md:items-center md:rounded-2xl md:border md:border-neutral-200 md:p-4 md:pb-4"
		>
			...
			<div class="flex h-fit w-full items-center justify-between md:hidden">
